Running the NTX software:

Dependencies: Java 7 or later needs to be installed first. Only the Oracle JVM
has been tested and supported.

There is no installation needed. Unpack the ntx-client.zip package and open a
shell in the resulting ntx directory. Execute the run.sh script if using Linux,
...
conf/ntx-default.properties file. All possible settings are listed, with
detailed explanation. If you decide to change any setting, do not edit
ntx-default.properties directly, but create a new conf/ntx.properties file
and only add to it the properties that need to be different from the default
values. You do not need to delete the defaults from ntx-default.properties, the
settings in ntx.properties override those in ntx-default.properties. This way,
when upgrading the software, you can safely overwrite ntx-default.properties
with the updated file from the new package, while your customizations remain
safe in the ntx.properties file.


Technical details:

The NTX software is a client-server application. It consists of a java server
...
ntx_db directory. When upgrading, you should not delete the old ntx_db
...
ntx_db, except that it will take some extra time to download the blockchain
from scratch.

The default NTX client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file ntx.log, which gets
...
package them in an ntx.jar file, replacing the existing one.
